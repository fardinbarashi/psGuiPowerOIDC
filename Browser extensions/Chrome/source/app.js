...
function addStepResult({ title, message, status = "Info", json }) {
  ...
  head.innerHTML = `
    <span class="step-badge">${STATUS_LABEL[status] || status}</span>
    <span class="step-title"></span>
    ${json !== undefined ? '<span class="json-badge">JSON</span>' : ""}
  `;
  ...
  if (json !== undefined) {
    ...
    head.querySelector(".json-badge").addEventListener("click", () => pre.classList.toggle("hidden"));
    row.appendChild(pre);
  }
...
function updateRedirectModeHint() {
  ...
  if (mode === "auto") {
    ...
    hint.innerHTML = uri
      ? `Automatiskt läge använder webbläsarens inloggningsfönster. Registrera denna redirect URI hos din IdP:<br><code>${uri}</code>`
      : "Automatiskt läge kräver identity-API:t (kunde inte hämta redirect URI).";
